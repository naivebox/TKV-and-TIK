...
# =============================================================================
# OMNI-ENGINE v7.4 - RESONANCE SWEEP (PROJEKT EVA)
# Hledání přesné frekvence (EM pulzu), která odemkne fázový zámek
# bez nutnosti extrémního kinetického tlaku.
# =============================================================================

kernel_code = r"""
#pragma OPENCL EXTENSION cl_khr_fp64 : enable

__kernel void tkv_sweep_step(
    ...
    const double dt, const int N, const double current_frequency, const int t_step)
{
    ...
    // Kinetický tlak je zde MINIMÁLNÍ (Simulace plynu při pokojové teplotě)
    ...
    // Aplikace Sweep Frekvence (Virtuální EM Výboj)
    // Otáčíme fází uzlů podle toho, jakou ladičku zrovna zkoušíme
    double angle = current_frequency * dt;
    ...
    // Fúze / Handover logik (Pokud se pod vlivem frekvence uzly překryjí)
    double intake = 0.0;
    if (t_step > 200) {
        double saturation = 0.5 * (1.0 - tanh(4.0 * (current_m / DEUTERIUM_TARGET - 0.98)));
        intake = 0.25 * saturation; // Zvýšená citlivost na spojení
    }
...
class ResonanceSweepScanner:
    def __init__(self, N=80):
        ...
        self.knl = cl.Kernel(self.prg, "tkv_sweep_step")
        self.results = []
    ...
    def run_sweep_test(self, test_frequency):
        ...
        # NEUTRON (s posunutou fází pro základní odpor)
        cx2, cy2, cz2 = self.N//2 + start_dist/2, self.N//2, self.N//2
        r2 = np.sqrt((x-cx2)**2 + (y-cy2)**2 + (z-cz2)**2)
        env2 = 16.0 * np.exp(-(r2**2)/15.0)
        phase2 = r2 - (x/self.N)*np.pi + np.pi/4 
        ...
        # 450 tiků na test každé frekvence
        for t in range(450):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     d_pr.data, d_pi.data, d_pr_n.data, d_pi_n.data, 
                     d_hm.data, np.float64(self.dt), np.int32(self.N), np.float64(test_frequency), np.int32(t))
        ...
        # Velmi jemná detekce (protože fúze bude jemná, ne násilná)
        status = "NEZNÁMÝ"
        if max_density >= 799.0:
            status = "SINGULARITA (Přetížení)"
        elif bridge_density < 30.0:
            status = "REPULZE (Frekvence nesedí)"
        elif bridge_density > 60.0 and max_density < 700.0:
            status = "REZONANČNÍ FÚZE (Handover OK!)"
        else:
            status = "ŠUM"
    ...
    def start_sweep(self):
        print("[*] Inicializuji TCD Ladičku (Resonance Sweep v7.4)...")
        print("[*] Hledám frekvenční okno pro nenásilnou fúzi v Levelu D")
        print("-" * 65)
        
        start_time = time.time()
        
        # Testujeme frekvence (např. od 0 do 20 s krokem 0.5)
        # Hledáme harmonickou shodu s mřížkou
        ...
        for freq in freq_range:
            ...
            if "FÚZE" in res["vysledek"]:
                print(f"[!] *** NALEZENA REZONANCE! *** Freq: {res['frekvence']} | Most: {res['stredovy_most']} | Max: {res['max_pnuti']}")
            elif res["frekvence"] % 5.0 == 0:
                print(f"[*] Skener na {res['frekvence']} Hz... (Výsledek: {res['vysledek']})")
    ...
    def export_data(self):
        filename = "tcd_resonance_sweep_export.csv"
...
if __name__ == "__main__":
    scanner = ResonanceSweepScanner(N=80)
    scanner.start_sweep()
