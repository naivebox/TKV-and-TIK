...
# =====================================================================
# TKV PROJEKT 1836: DYNAMICKÁ VAZEBNÁ KONSTANTA (RUNNING COUPLING)
# Cíl: Dosažení reálného poměru hmotnosti Proton/Elektron
# =====================================================================
...
__kernel void tkv_dynamic_step(
    ...
    const double coupling_strength) // Nový parametr pro sílu lepidla!
{
    ...
    const double LAMBDA_W = 0.0833333333; // Základní odpor (EM síla)
    ...
    // =============================================================
    // DYNAMICKÁ KONSTANTA (RUNNING COUPLING)
    // =============================================================
    double local_density = pr*pr + pi*pi;
    double local_curvature = sqrt(lap_r*lap_r + lap_i*lap_i); // Velikost pnutí
    
    // Konstanta roste úměrně tomu, jak moc se prostor křiví a kolik je tam hmoty
    // Pokud je prostor klidný (vakuum/elektron), člen v závorce je blízko nule.
    double dynamic_lambda = LAMBDA_W + (coupling_strength * local_density * local_curvature);
    
    // Maxwellova indukce nyní používá zesílenou (lokální) konstantu
...
class TKV_Analyzer_1836:
    def __init__(self, size=64):
        ...
        print(f"[+] Projekt 1836 online na: {dev.name}")
        ...
        self.electron_energy_ref = 3683.0 # Fallback
    ...
    def tare_vacuum(self, ticks=500):
        ...
        for t in range(ticks):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     ...
                     np.int32(0), np.float64(0.0), np.float64(0.0))
    ...
    def measure_electron(self):
        ...
        for t in range(2000):
            # U elektronu je coupling strength téměř irelevantní, ale necháme ho tam (např. 5.0)
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     self.d_pr.data, self.d_pi.data, self.d_pr_n.data, self.d_pi_n.data, 
                     self.d_ax.data, self.d_ay.data, self.d_az.data, self.d_total_energy.data, 
                     np.float64(self.dt), np.int32(self.N), np.float64(self.global_time),
                     np.int32(1), np.float64(0.0), np.float64(5.0))
    ...
    def run_proton_fusion(self, coupling_strength):
        print(f"\n[*] FÁZE 2: Zrod Protonu (Dynamická konstanta: {coupling_strength})")
        self.d_ax.fill(0.0); self.d_ay.fill(0.0); self.d_az.fill(0.0); self.d_total_energy.fill(0.0)
        ...
        for t in range(1501):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     ...
                     self.d_ax.data, self.d_ay.data, self.d_az.data, self.d_total_energy.data, 
                     np.float64(self.dt), np.int32(self.N), np.float64(self.global_time),
                     np.int32(physics_mode), np.float64(vacuum_heat), np.float64(coupling_strength))
            self.d_pr, self.d_pr_n = self.d_pr_n, self.d_pr
            self.d_pi, self.d_pi_n = self.d_pi_n, self.d_pi
        ...
        # KROK 2: Plné 3D + Var vakua (Indukce masivního pole)
        physics_mode = 1
        vacuum_heat = 0.015
        for t in range(3001):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
...
if __name__ == "__main__":
    ...
    # Hledáme tu správnou hodnotu zesílení pole (Running Coupling).
    # Zkusíme nastavit sílu na 15.0. Pokud to bude málo/moc, budeme ji ladit.
    analyzer.run_proton_fusion(coupling_strength=15.0)
