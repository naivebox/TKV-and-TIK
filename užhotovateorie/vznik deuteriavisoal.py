    ...
    const double dt, const int N, const int t_step)
{
    ...
    // Jemný kinetický drift (Přibližování uzlů v prvních 200 ticích)
    double drift_r = 0.0; double drift_i = 0.0;
    double momentum = 1.2;
    
    if (t_step < 200) {
        if (x < N/2 - 1) { // Levý uzel (Proton) letí doprava
            drift_r = momentum * (psi_r[id_xm] - pr);
            drift_i = momentum * (psi_i[id_xm] - pi);
        } else if (x > N/2 + 1) { // Pravý uzel (Neutron) letí doleva
...
class HandoverVisualizer:
    def __init__(self, N=100):
        ...
        print("[*] Inicializuji fúzní komoru Level D pro vizualizaci...")
        x, y, z = np.indices((N, N, N))
        
        # Startovní vzdálenost 20 voxelů (aby byl vidět krásný proces přiblížení)
        start_dist = 20.0 
        
        # PROTON (Vlevo)
        cx1, cy1, cz1 = N//2 - start_dist/2, N//2, N//2
        r1 = np.sqrt((x-cx1)**2 + (y-cy1)**2 + (z-cz1)**2)
        env1 = 15.0 * np.exp(-(r1**2)/8.0)
        phase1 = r1 + (x/N)*np.pi
        
        # NEUTRON (Vpravo - fázově invertován pro budoucí zámek)
        cx2, cy2, cz2 = N//2 + start_dist/2, N//2, N//2
        r2 = np.sqrt((x-cx2)**2 + (y-cy2)**2 + (z-cz2)**2)
        env2 = 16.0 * np.exp(-(r2**2)/8.0) # Neutron je nepatrně hmotnější
        ...
        # Ořez pro detailní pohled na jádro
        self.zoom = 30
        self.im = self.ax.imshow(np.zeros((self.zoom*2, self.zoom*2)), cmap='plasma', origin='lower', vmin=0, vmax=500)
        
        self.ax.set_title("TCD: Topologická směna (Syntéza Deuteria)", color='cyan', pad=15, fontsize=14)
        self.ax.axis('off')
        ...
        for t in range(steps):
            ...
            if t % 10 == 0: # Rychlé překreslování pro plynulou animaci
                ...
                if t < 200:
                    ...
                    sync = 100.0
                    defect = ((t - 200) / 200) * 2.22 # Nárůst vazebné energie
                else:
                ...
                if t > 50:
                    self.contours = self.ax.contour(slice_smooth, levels=[100, 250, 400], colors=['cyan', 'magenta', 'white'], alpha=0.5, linewidths=1.0)
