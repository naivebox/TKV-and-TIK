...
# =====================================================================
# OMNI-ENGINE v5.2: PROJEKT HARMONIKA - HADRON STRING TEST
# Simulace tří protonů v řadě pro ověření lokální stability
# =====================================================================

kernel_code = r"""
#pragma OPENCL EXTENSION cl_khr_fp64 : enable

__kernel void tkv_hadron_string_step(
    ...
    const double dt, const int N, const double time)
{
    ...
    // Sifon je nyní aplikován na CELOU mřížku jako plošný filtr (bez fixního středu)
    // aby se protony mohly tvořit kdekoliv.
    double nr = pr + (0.05 * lap_r * isolation * dt) + (vacuum_intake * pr * dt);
    double ni = pi + (0.05 * lap_i * isolation * dt) + (vacuum_intake * pi * dt);
...
class MultiHadronVisualizer:
    def __init__(self, size=64):
        ...
        self.knl = cl.Kernel(self.prg, "tkv_hadron_string_step")
        ...
        self.fig.colorbar(self.im, label='Hustota Hadronů')
        self.ax.set_title("OMNI-ENGINE v5.2: Hadronový řetězec v řadě")

    def inject_string(self):
        """Vstřikuje 3 protony v řadě podél osy Y."""
        print("[*] Vstřikuji protonový řetězec (3 uzly)...")
    ...
    def update_visuals(self, t):
        ...
        slice_2d = mass_3d[self.N // 2, :, :] 
        self.im.set_data(slice_2d)
        self.im.set_clim(vmin=0, vmax=np.max(slice_2d))
        self.ax.set_title(f"Hadronový řetězec | Tik: {t} | Max hmota: {np.max(slice_2d):.2f}")
        self.fig.canvas.draw()
        self.fig.canvas.flush_events()

    def run(self, total_ticks=5000):
        self.inject_string()
...
if __name__ == "__main__":
    sim = MultiHadronVisualizer(size=64)
    sim.run()
