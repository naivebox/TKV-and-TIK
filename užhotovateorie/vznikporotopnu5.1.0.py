...
# =====================================================================
# TKV PROJEKT 1836: TOPOLOGICKÁ HOMEOSTÁZA
# Zákaz umělých limitů! Hmotnost je omezena POUZE fázovým skluzem
# a topologickou rezonancí (ztrátou koherence mřížky).
# =====================================================================

kernel_code = r"""
#pragma OPENCL EXTENSION cl_khr_fp64 : enable

__kernel void tkv_homeostasis_step(
    ...
    const int physics_mode, const double base_vacuum_heat)
{
    ...
    const double LAMBDA_W = 0.0833333333; // Základní síla vakua
    const double ALPHA_G = 1.19998161486; // Konstanta jemné struktury
    ...
    double local_curvature = sqrt(lap_r*lap_r + lap_i*lap_i);

    // ====================================================================
    // 1. HORIZONT SYNCHRONIZACE (Topologická Koherence)
    // Jak moc fáze tohoto uzlu "ladí" se zbytkem částice?
    // ====================================================================
    double sum_r = rxp + rxm + ryp + rym + rzp + rzm;
    double sum_i = ixp + ixm + iyp + iym + izp + izm;
    
    // Velikost vektorového součtu (ruší se, pokud jsou fáze rozházené)
    double vector_sum_mag = sqrt(sum_r*sum_r + sum_i*sum_i);
    
    // Součet absolutních hodnot (co by bylo, kdyby byly dokonale sladěné)
    double abs_sum = sqrt(rxp*rxp+ixp*ixp) + sqrt(rxm*rxm+ixm*ixm) + 
                     sqrt(ryp*ryp+iyp*iyp) + sqrt(rym*rym+iym*iym) + 
                     sqrt(rzp*rzp+izp*izp) + sqrt(rzm*rzm+izm*izm);
                     
    // Koherence je číslo od 0.0 (chaos) do 1.0 (dokonalá synchronizace)
    double coherence_factor = (abs_sum > 0.0001) ? (vector_sum_mag / abs_sum) : 1.0;

    // ====================================================================
    // 2. NYQUISTŮV LIMIT PROSTORU (Fázový skluz)
    // Natahuje se prostor příliš? Přesáhl rozdíl fází 180 stupňů?
    // ====================================================================
    double center_phase = atan2(pi, pr);
    double max_phase_diff = 0.0;
    
    // Funkce pro nejkratší úhlovou vzdálenost (wrapping na hranici +-PI)
    double diff_xp = fabs(center_phase - atan2(ixp, rxp)); if(diff_xp > PI) diff_xp = 2.0*PI - diff_xp;
    double diff_xm = fabs(center_phase - atan2(ixm, rxm)); if(diff_xm > PI) diff_xm = 2.0*PI - diff_xm;
    double diff_yp = fabs(center_phase - atan2(iyp, ryp)); if(diff_yp > PI) diff_yp = 2.0*PI - diff_yp;
    double diff_ym = fabs(center_phase - atan2(iym, rym)); if(diff_ym > PI) diff_ym = 2.0*PI - diff_ym;
    double diff_zp = fabs(center_phase - atan2(izp, rzp)); if(diff_zp > PI) diff_zp = 2.0*PI - diff_zp;
    double diff_zm = fabs(center_phase - atan2(izm, rzm)); if(diff_zm > PI) diff_zm = 2.0*PI - diff_zm;

    // Najdeme nejhorší fázové napětí v okolí uzlu
    max_phase_diff = fmax(diff_xp, fmax(diff_xm, fmax(diff_yp, fmax(diff_ym, fmax(diff_zp, diff_zm)))));
    
    // Pokud se napětí blíží PI/2, geometrie praská. Penalizace roste od 0 do 1.
    // Pro bezpečí to modelujeme plynulou křivkou.
    double phase_stress = max_phase_diff / (PI / 2.0);
    double phase_gradient_penalty = (phase_stress > 1.0) ? 1.0 : (phase_stress * phase_stress);
    ...
    // Lepidlo: Už NENÍ uměle násobeno (např. * 183.6)!
    // Lepidlo je násobeno pouze vlastní hustotou, dokud se uzel nezačne lámat (phase_gradient_penalty)
    // a dokud neztratí synchronizaci (coherence_factor).
    double geometric_multiplier = local_density * coherence_factor * (1.0 - phase_gradient_penalty);
    double dynamic_lambda = LAMBDA_W * (1.0 + geometric_multiplier);
    ...
    // VAR VAKUA (Příjem potravy)
    // Pokud je koherence malá nebo napětí velké, vacuum_intake klesá k nule! Částice přestane sát.
    double vacuum_intake = base_vacuum_heat * coherence_factor * (1.0 - phase_gradient_penalty);
    double jitter = sin((double)x*1.618 + (double)y*3.14 + (double)z*2.71 + time*15.0) * vacuum_intake;

    // Pokud je fázový skluz kritický (penalty > 0.8), přidáme i jemný disipativní rozpad,
    // aby se částice "odfoukla" a nepraskla celá mřížka.
    double dissipation = (phase_gradient_penalty > 0.8) ? (0.05 * pr * dt) : 0.0;

    double nr = pr + (0.05 * lap_r * dt) + (tenze * pr * dt) - (siphon_friction * pr * dt) + jitter - dissipation;
    double ni = pi + (0.05 * lap_i * dt) + (tenze * pi * dt) - (siphon_friction * pi * dt) + jitter - dissipation;
...
class TKV_Analyzer_Homeostasis:
    def __init__(self, size=64): 
        ...
        self.knl = cl.Kernel(self.prg, "tkv_homeostasis_step")
        
        print(f"[+] Projekt 1836 (Topologická Homeostáza) online na: {dev.name}")
    ...
    def measure_electron(self):
        print("\n[*] FÁZE 1: Měření referenčního Elektronu...")
        ...
        print(f"[+] Skutečná hmotnost elektronu: {self.electron_energy_ref:.2f}")

    def run_proton_fusion(self):
        print(f"\n[*] FÁZE 2: Hledání přirozené hmotnosti Protonu (žádné umělé limity)")
        self.d_ax.fill(0.0); self.d_ay.fill(0.0); self.d_az.fill(0.0); self.d_total_energy.fill(0.0)
        ...
        # Jdeme do 20000 tiků. Tady by měla homeostáza zajistit zamrznutí hodnoty bez cizí pomoci!
        for t in range(18501):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     self.d_pr.data, self.d_pi.data, self.d_pr_n.data, self.d_pi_n.data, 
                     self.d_ax.data, self.d_ay.data, self.d_az.data, self.d_total_energy.data, 
                     np.float64(self.dt), np.int32(self.N), np.float64(self.global_time),
            ...
            if t > 0 and t % 1000 == 0:
                ...
                renorm_p = raw_p - self.vacuum_energy_tara
                ratio = renorm_p / self.electron_energy_ref
                print(f"3D Pě| {t+1500:05d}  | {renorm_p:34.2f} | {ratio:11.2f}")

        print("-" * 65)
        print(f"[*] PŘIROZENÁ HMOTNOST PROTONU: {ratio:.2f}")

if __name__ == "__main__":
    analyzer = TKV_Analyzer_Homeostasis(size=64) 
    analyzer.tare_vacuum()
    analyzer.measure_electron()
    # Metoda run_proton_fusion už nepřijímá žádný umělý multiplikátor!
    analyzer.run_proton_fusion()
