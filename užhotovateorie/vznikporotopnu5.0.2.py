...
# =====================================================================
# TKV PROJEKT 1836: PŘIROZENÉ ŠKÁLOVÁNÍ
# Cíl: Odstranění umělých klecí a použití Phi pro dynamickou vazbu
# =====================================================================
...
__kernel void tkv_dynamic_step(
    ...
    const double coupling_multiplier) // Nyní násobíme přirozenou Lambda!
{
    ...
    const double SIGMA = 2.61803398875;     // Phi^2
    const double LAMBDA_W = 0.0833333333;   // Základní síla
    ...
    // Jemnější Sifon, aby netlumil proton tak drasticky
    ...
    // =============================================================
    // PŘIROZENÁ DYNAMICKÁ VAZBA
    // =============================================================
    double local_density = pr*pr + pi*pi;
    
    // Místo obří konstanty (250) jemně násobíme základní LAMBDA_W
    // podle toho, jaká je hustota uzlu, a násobíme ji dodaným koeficientem.
    // Tím získáme plynulý růst pole.
    double dynamic_lambda = LAMBDA_W * (1.0 + (coupling_multiplier * local_density));
...
class TKV_Analyzer_1836:
    def __init__(self, size=64): # Vrátíme na 64 pro rychlejší ladění
        self.N = size
        self.dt = 0.005
        ...
        print(f"[+] Projekt 1836 (Přirozená Vazba) online na: {dev.name} [Vesmír: {self.N}^3]")
    ...
    def tare_vacuum(self, ticks=500):
        ...
        for t in range(ticks):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     ...
                     np.int32(0), np.float64(0.0), np.float64(0.0)) # Multiplier 0 pro vakuum
    ...
    def measure_electron(self):
        ...
        for t in range(2000):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     ...
                     self.d_ax.data, self.d_ay.data, self.d_az.data, self.d_total_energy.data, 
                     np.float64(self.dt), np.int32(self.N), np.float64(self.global_time),
                     np.int32(1), np.float64(0.0), np.float64(0.0)) # Elektron má multiplier nula!
    ...
    def run_proton_fusion(self, multiplier):
        print(f"\n[*] FÁZE 2: Zrod Protonu (Násobič silné interakce: {multiplier})")
        self.d_ax.fill(0.0); self.d_ay.fill(0.0); self.d_az.fill(0.0); self.d_total_energy.fill(0.0)
        ...
        for t in range(1501):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     ...
                     self.d_ax.data, self.d_ay.data, self.d_az.data, self.d_total_energy.data, 
                     np.float64(self.dt), np.int32(self.N), np.float64(self.global_time),
                     np.int32(physics_mode), np.float64(vacuum_heat), np.float64(multiplier))
            self.d_pr, self.d_pr_n = self.d_pr_n, self.d_pr
            self.d_pi, self.d_pi_n = self.d_pi_n, self.d_pi
        ...
        physics_mode = 1
        vacuum_heat = 0.010 # Mírnější var vakua, ať se neroztrhá
        for t in range(4001):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     self.d_pr.data, self.d_pi.data, self.d_pr_n.data, self.d_pi_n.data, 
                     self.d_ax.data, self.d_ay.data, self.d_az.data, self.d_total_energy.data, 
...
if __name__ == "__main__":
    # Vracíme se na mřížku 64 pro rychlejší zpětnou vazbu
    analyzer = TKV_Analyzer_1836(size=64) 
    analyzer.tare_vacuum()
    analyzer.measure_electron()
    
    # Použijeme geometrický násobič. 
    # Místo náhodného čísla zkusíme jako násobič samotný Zlatý řez (2.618) nebo jeho mocniny.
    # Např. 2.618 * 2.618 * 2.618 (Protože srážíme 3 kvarky!) = cca 17.94
    analyzer.run_proton_fusion(multiplier=18.0)
