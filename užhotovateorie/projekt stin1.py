...
# =====================================================================
# OMNI-ENGINE v5.2 VISUALIZER: PROJEKT HARMONIKA
# Vizualizace centrálního řezu 3D mřížky v reálném čase
# =====================================================================

kernel_code = r"""
#pragma OPENCL EXTENSION cl_khr_fp64 : enable

__kernel void tkv_hadron_viz_step(
    ...
    const double dt, const int N, const double time)
{
    ...
    const double TARGET_RATIO = 275.4258; // Nastaveno na pozorovanou homeostázu
    ...
    // 2. GLUONOVÁ IZOLACE (Potlačení Laplaciánu uvnitř uzlu)
    ...
    double friction = 0.3 * exp(-dist / 2.0);

    double nr = pr + (0.05 * lap_r * isolation * dt) + (vacuum_intake * pr * dt) - (friction * pr * dt);
    double ni = pi + (0.05 * lap_i * isolation * dt) + (vacuum_intake * pi * dt) - (friction * pi * dt);
    ...
    // Uložíme hmotnost pro vizualizaci
    h_mass[i] = nm;
}
"""

class HadronVisualizer:
    def __init__(self, size=64):
        ...
        self.knl = cl.Kernel(self.prg, "tkv_hadron_viz_step")
        ...
        self.im = self.ax.imshow(np.zeros((self.N, self.N)), cmap='inferno', origin='lower', extent=[-self.N/2, self.N/2, -self.N/2, self.N/2])
        self.fig.colorbar(self.im, label='Lokální hmotnost (Energetická hustota)')
        self.ax.set_title("OMNI-ENGINE v5.2: Zrození Protonu (Centrální řez)")
        self.text = self.ax.text(0.02, 0.95, '', transform=self.ax.transAxes, color='white', fontweight='bold')
    ...
    def inject(self):
        ...
        envelope = 2.0 + 8.0 * np.exp(-(r**2) / 10.0)
    ...
    def update_visuals(self, t):
        ...
        slice_2d = mass_3d[self.N // 2, :, :] # Centrální řez
        
        self.im.set_data(slice_2d)
        self.im.set_clim(vmin=0, vmax=np.max(slice_2d)) # Dynamický rozsah
        
        max_m = np.max(slice_2d)
        self.text.set_text(f"Tik: {t} | Max hmota: {max_m:.2f}")
        
        self.fig.canvas.draw()
        self.fig.canvas.flush_events()

    def run(self, total_ticks=10000):
        self.inject()
        for t in range(total_ticks):
            self.knl(self.queue, (self.N, self.N, self.N), None, 
                     ...
                     np.float64(self.dt), np.int32(self.N), np.float64(0.0))
