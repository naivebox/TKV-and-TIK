...
# + MULTI-DEVICE SUPPORT (Automatická detekce GPU nebo CPU)
# =====================================================================
...
class HydrogenSimulator:
    def __init__(self, size=64):
        ...
        # PŘIPOJENÍ K NEJLEPŠÍMU DOSTUPNÉMU ZAŘÍZENÍ (GPU -> CPU)
    ...
    def _choose_device(self):
        """Automaticky detekuje a vybere nejlepší OpenCL zařízení (GPU priorita)."""
        platforms = cl.get_platforms()
        gpus = []
        cpus = []
        
        # Prohledá všechny platformy (AMD, NVIDIA, Intel, PoCL...)
        for p in platforms:
            try:
        ...
        if gpus:
            ...
            # Fallback na cokoliv, co je dostupné
            best_dev = platforms[0].get_devices()[0]
            dev_type = "Neznámé/Jiné"
            
        print(f"[+] OMNI-ENGINE v5.4.3: Quark-Trefoil Ready.")
        print(f"[+] Zařízení připojeno: {best_dev.name} ({dev_type})")
        return best_dev

    def inject_atom(self):
        print("[*] Vstřikuji fázově propletený Trefoil a elektronový mrak...")
        x, y, z = np.indices((self.N, self.N, self.N))
        dx, dy, dz = x - self.N//2, y - self.N//2, z - self.N//2
        r = np.sqrt(dx**2 + dy**2 + dz**2)
        
        p_env = 15.0 * np.exp(-(r**2) / 8.0)
        p1 = (dx/self.N)*np.pi*150
        p2 = (dy/self.N)*np.pi*150 + (2.0*np.pi/3.0)
        p3 = (dz/self.N)*np.pi*150 + (4.0*np.pi/3.0)
        
        e_env = 1.5 * np.exp(-((r - 22.0)**2) / 35.0) 
        e_phase = np.arctan2(dy, dx) * 2.0
        
        pr = p_env * (np.cos(p1) + np.cos(p2) + np.cos(p3)) + e_env * np.cos(e_phase)
        pi = p_env * (np.sin(p1) + np.sin(p2) + np.sin(p3)) + e_env * np.sin(e_phase)
